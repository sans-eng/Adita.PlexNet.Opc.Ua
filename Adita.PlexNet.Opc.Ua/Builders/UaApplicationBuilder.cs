...
namespace Adita.PlexNet.Opc.Ua.Builders
{
    /// <summary>
    ...
    public class UaApplicationBuilder
    {
        ...
        {
            configureLoggerFactoryActions.Add(configureDelegate ?? throw new ArgumentNullException(nameof(configureDelegate)));
            return this;
        }
        ...
        public UaApplicationBuilder AddMappedEndpoint(string requestedUrl, string endpointUrl, string? securityPolicyUri = null)
        {
            ...
            mappedEndpoints.Add(new MappedEndpoint { RequestedUrl = requestedUrl, Endpoint = new EndpointDescription { EndpointUrl = endpointUrl, SecurityPolicyUri = securityPolicyUri } });
            return this;
        }
        ...
        public UaApplication Build()
        {
            ...
            foreach (var action in configureLoggerFactoryActions)
            {
                action(loggerFactory);
            }

            var options = this.options ?? new UaApplicationOptions();
            foreach (var action in configureOptionsActions)
            {
                action(options);
            }
