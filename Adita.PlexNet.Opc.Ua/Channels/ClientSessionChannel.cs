        ...
        private void ThrowOnInvalidSessionServerCertificate(byte[]? sessionCertificate)
        {
            ...
            if (compareCertificates)
            {
                var isValid = RemoteEndpoint.ServerCertificate == null || sessionCertificate == null
                    ? RemoteEndpoint.ServerCertificate == null && sessionCertificate == null // Valid if both certificates are null
                    : RemoteEndpoint.ServerCertificate.SequenceEqual(sessionCertificate); // Valid if both certificates are equal
